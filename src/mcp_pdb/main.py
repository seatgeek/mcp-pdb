...
def start_debug(
    ...
    try:
                ...
                try:
                    with open(os.path.join(project_root, "pyproject.toml"), "r") as f:
                        ...
                        if "[tool.uv]" in content:
                            print(
                                "Found [tool.uv] in pyproject.toml, assuming uv project."
                            )
                            use_uv = True
                except (IOError, OSError):
                    pass
        ...
        if (
            not use_uv
            and poetry_path
            and os.path.exists(os.path.join(project_root, "pyproject.toml"))
        ):
            # Check for Poetry-specific indicators
            if os.path.exists(
                os.path.join(project_root, "poetry.lock")
            ) or has_poetry_config(project_root):
        ...
        if use_uv:
            ...
            print(
                "Warning: No uv, poetry, or standard venv detected in project root. Using system Python/pytest."
            )
        ...
        if current_file in breakpoints and breakpoints[current_file]:
            print(
                f"Restoring {len(breakpoints[current_file])} breakpoints for {rel_file_path}..."
            )
            # Use relative path for consistency in breakpoint commands
            try:
                bp_rel_path = os.path.relpath(current_file, project_root)
                if bp_rel_path == ".":
                    bp_rel_path = os.path.basename(current_file)
            except ValueError:
...
def set_breakpoint(file_path: str, line_number: int) -> str:
    ...
    abs_file_path = os.path.abspath(
        os.path.join(current_project_root, file_path)
...
def clear_breakpoint(file_path: str, line_number: int) -> str:
    ...
    if not os.path.exists(abs_file_path):
        ...
        if not os.path.exists(abs_file_path):
            ...
            if (
                abs_file_path in breakpoints
                and line_number in breakpoints[abs_file_path]
            ):
...
def restart_debug() -> str:
    ...
    start_result = start_debug(
        file_path=file_to_debug, use_pytest=use_pytest_flag, args=args_to_use
    )
